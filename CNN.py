...
x = keras.layers.Conv2D(filters=8, kernel_size=(4,4), strides=(1, 1), activation='relu', padding='same')(two_inputs)
x = keras.layers.MaxPooling2D(pool_size=(4, 4), strides=(2, 2), padding='same')(x)
x = keras.layers.BatchNormalization()(x)

x = keras.layers.Conv2D(filters=16, kernel_size=(4, 4), padding='same', activation='relu')(x)
x = keras.layers.MaxPooling2D(pool_size=(4, 4), strides=(2, 2), padding='same')(x)
x = keras.layers.BatchNormalization()(x)

x = keras.layers.Conv2D(filters=32, kernel_size=(4, 4), padding='same', activation='relu')(x)
x = keras.layers.MaxPooling2D(pool_size=(4, 4), strides=(2, 2),padding='same')(x)
x = keras.layers.BatchNormalization()(x)

x = keras.layers.Conv2D(filters=64, kernel_size=(4, 4), padding='same', activation='relu', kernel_regularizer =tf.keras.regularizers.l2( l=0.01))(x)
x = keras.layers.MaxPooling2D(pool_size=(4, 4), strides=(2, 2),padding='same')(x)
x = keras.layers.BatchNormalization()(x)

x = keras.layers.Conv2D(filters=128, kernel_size=(4, 4),padding='same', activation='relu', kernel_regularizer =tf.keras.regularizers.l2( l=0.01))(x)
x = keras.layers.MaxPooling2D(pool_size=(4, 4), strides=(2, 2),padding='same')(x)
x = keras.layers.BatchNormalization()(x)
...
save_model(model, "CNN_high.h5")

#model = load_model('Linear_filtered_both.h5', custom_objects={"L2_loss": L2_loss })